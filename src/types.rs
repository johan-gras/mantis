...
impl Order {
    ...
    pub fn market(symbol: impl Into<String>, side: Side, quantity: f64, timestamp: DateTime<Utc>) -> Self {
        Self {
            symbol: symbol.into(),
            side,
            quantity,
...
mod tests {
    ...
    #[test]
    fn test_trade_pnl() {
        let mut trade = Trade::open("AAPL", Side::Buy, 100.0, 150.0, sample_timestamp(), 1.0, 0.01);
    ...
    fn test_trade_short_pnl() {
        let mut trade = Trade::open("AAPL", Side::Sell, 100.0, 160.0, sample_timestamp(), 1.0, 0.0);
